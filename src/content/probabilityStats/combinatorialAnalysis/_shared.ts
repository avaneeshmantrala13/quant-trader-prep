...
/**
 * Shared generator scaffolding for the Probability & Statistics →
 * **Combinatorial Analysis** subcategory. Mirrors the helpers baked into the
 * sibling subcategories (assemble/shuffle MC choices, dedupe numeric
 * commonErrors, mix generator pools) so every family generator file stays
 * consistent.
 *
...
export function numericErrors(
  ...
  const push = (raw: FractionType | number, feedback: string) => {
    ...
    if (v < 0) return; // the app's numeric answers are non-negative
