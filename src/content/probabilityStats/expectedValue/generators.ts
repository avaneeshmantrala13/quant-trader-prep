...
export function buildWalkReachInstance(
  ...
  // Avoid the exact midpoint i = N/2: there the answer is exactly ½, which
  // collides with the "symmetry guess" distractor AND the (N−i)/N complement,
  // collapsing the option set below four.
  let i = N / 2;
  while (i === N / 2) i = rng.int(Math.floor(N / 4), Math.floor((3 * N) / 4));
...
export function buildWalkDurationInstance(
  ...
  const N = rng.pick([10, 20, 50, 100]);
  // Avoid i = N/2: there (N−i)² equals the answer i·(N−i), collapsing the
  // option set below four.
  let i = N / 2;
  while (i === N / 2) i = rng.int(Math.floor(N / 4), Math.floor((3 * N) / 4));
