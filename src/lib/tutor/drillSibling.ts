 ...
 *  - `numeric-authored`: a single fixed authored question with no parametric
 *    variation and no sibling pool, so a "same kind, different numbers" instance
 *    genuinely cannot be produced. We return `null`; the hint ladder then omits
 *    the worked-sibling rung entirely (header ⇔ steps invariant) and falls back
 *    to the next meaningful hint.
 */

/**
 * A throwaway generator-backed `Level` whose `numericGenerator` is the drill
 * item's own hard-OA family (verifier-backed), or `null` when the item has no
 * parametric family to regenerate from (authored singletons).
 */
export function drillWorkedSiblingLevel(
  item: MaterializedNumericItem,
): Level | null {
  if (item.item.kind !== "numeric-adapter") return null;
