...
/* ========================================================================== */
/*  Counterparty (informed trader)                                             */
/* ========================================================================== */

/**
 * The counterparty acts on a TIGHT quote. It knows `trueValue`.
 *  • If ask < trueValue → it BUYS your ask (you SELL → go shorter). The cheaper
 *    your ask relative to true, the more size it presses (the stale-market
 *    lesson): edge measured in "max-spread units".
 *  • If bid > trueValue → it SELLS to your bid (you BUY → go longer).
 *  • Otherwise (true value inside [bid, ask]) → it passes, UNLESS the market is
 *    lopsided enough that one side is still an edge; then it nibbles 1 lot.
 * `aggression` scales how much size it will take when it has edge.
 */
export function counterpartyTight(
  ...
  const askEdge = trueValue - q.ask; // >0 → buying your ask is +EV for them
  const bidEdge = q.bid - trueValue; // >0 → selling to your bid is +EV for them

  // Pick the profitable side (larger positive edge).
  if (askEdge > 0 && askEdge >= bidEdge) {
  ...
  // True value inside the market → they pass (good two-sided quote).
  return { fill: null, chatter: "Fair enough — no trade." };
}
...
/**
 * Generate between-round coaching from the fill history, encoding the doc's
 * lessons: after being lifted, your NEXT quote on that side should be a better
 * price (skew up when you keep selling); don't hold a symmetric market around a
 * stale mid; when you believe the opponent is offside, add SIZE rather than
 * chasing price.
 */
export function coachAfterRound(
  ...
  if (f.side === "sell" && sells.length >= 2) {
    ...
    if (notHigher) {
      return {
        headline: "You keep selling — but you're not lifting your market.",
        detail: `Your last sell was at ${fmt(lastTwo[0].price)}; your next ask should be HIGHER. The opponent is happy to keep buying a stale ask. Skew your whole market up, and if you're confident it's cheap, show more size on the bid.`,
      };
    }
  }
  if (f.side === "buy" && buys.length >= 2) {
    ...
    if (notLower) {
      return {
        headline: "You keep buying — drop your market.",
        detail: `Your last buy was at ${fmt(lastTwo[0].price)}; your next bid should be LOWER. You're paying up into a market that's drifting against you — move the whole thing down.`,
      };
    }
  }
  if (f.size >= 3) {
    return {
      ...
      detail: `They took ${f.size} lots at ${fmt(f.price)} — that's them telling you the price was well off. Don't nudge; jump your market hard to the side that stops the bleeding.`,
    };
  }
  return null;
}
