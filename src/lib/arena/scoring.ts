 ...
 *  - Penalty sprint: +1 correct / −1 wrong, "skips free" toggle (skip scores 0
 *    when free, −1 when strict). This mirrors a real timed arithmetic sprint screen where
...
/**
 * Penalty rule: +1 per correct, −1 per wrong. A skip scores 0 when `skipsFree`,
...
/**
 * Score a run under whichever rule its preset implies: `penalty` ⇒ +1/−1
 * penalty scoring (honoring `skipsFree`); otherwise Zetamac count. This is the single
